...
title_for() {
  case "$1" in
    chessa)    echo "Mobile & CPS — Modulo Chessa (Appunti Esame)" ;;
    paganelli) echo "Mobile & CPS — Modulo Paganelli (Appunti Esame)" ;;
    *)         echo "Mobile & CPS — $1 (Appunti Esame)" ;;
  esac
}
...
for mod in chessa paganelli; do
  ...
  ( cd "$mod" && tectonic -X compile appunti_esame.tex && tectonic -X compile appunti_esame.tex )

  echo "==> [$mod] Done: $mod/appunti_esame.pdf"
...
ls -lh chessa/appunti_esame.pdf paganelli/appunti_esame.pdf
