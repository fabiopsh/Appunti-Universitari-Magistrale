...
flowchart TD
    A[Cloud Admin: richiesta espansione storage] --> B{Approvazione necessaria?}
    B -- Sì --> C[Richiedi approvazione]
    C --> D[Approvazione concessa]
    D --> E[Provisioning storage]
    B -- No --> E
    E --> F[Aggiorna CMDB/CMS]
    F --> G[Notifica completamento]
```
...
%%{init: {"flowchart": {"useMaxWidth": true}}}%%
flowchart TD
    A[Rilevazione evento] --> B[Log incidente]
    B --> C{Incidente reale?}
    C -- No: percezione errata --> D[Chiudi ticket]
    C -- Sì --> E[Identifica causa diretta]
    E --> F[Risolvi entro SLA time]
    F --> G[Chiudi incidente]
    G --> H{Incidente ricorrente?}
    H -- No --> I[Fine]
    H -- Sì --> J[Apri ticket problema]
    J --> K[Problem Management: analisi root cause]
    K --> L{Serve un change?}
    L -- Sì --> M[Richiedi Change Management]
    L -- No --> N[Fix software/config]
    M --> O[Risolvi problema]
    N --> O
    O --> P[Chiudi ticket problema]
```
...
>
> - Qual è la differenza tra incidente e problema? Fai un esempio.
> - Perché il change management è fondamentale nelle operations?
> - Cosa si intende per predictive failure e come si integra nel problem management?
> - Perché spegnere un datacenter è una decisione rischiosa?
> - Come funziona il tiering dello storage per le VM offline?
> - Cosa si intende per showback? In cosa differisce dal chargeback?
> - Perché il bilanciamento è più importante delle singole prestazioni dei componenti?
> - Come si manifesta il capacity management nell'overbooking della CPU?
