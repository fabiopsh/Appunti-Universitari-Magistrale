...
\usepackage{longtable,booktabs,array}
\usepackage{makecell}
\usepackage[font=it,labelfont={bf,it}]{caption}
...
\mainmatter
\chapter{Capitolo 1 --- Introduzione e Sistemi
Peer-to-Peer}\label{capitolo-1-introduzione-e-sistemi-peer-to-peer}

\section{Capitolo 1 - Introduzione e Sistemi
Peer-to-Peer}\label{capitolo-1---introduzione-e-sistemi-peer-to-peer}

\subsection{Il Paradigma Peer-to-Peer}\label{il-paradigma-peer-to-peer}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={A sinistra una rete centralizzata con un singolo server (Single Point of Failure), a destra un'architettura Peer-to-Peer totalmente distribuita e interconnessa.}]{images/p2p_vs_centralized.png}}
\caption{A sinistra una rete centralizzata con un singolo server (Single
Point of Failure), a destra un'architettura Peer-to-Peer totalmente
distribuita e interconnessa.}
\end{figure}
...
\subsubsection{Condivisione di Risorse}\label{condivisione-di-risorse}
...
\subsection{Blockchain: Cos'è e Quando Usarla
(Panoramica)}\label{blockchain-cosuxe8-e-quando-usarla-panoramica}
...
\subsubsection{Bitcoin ed Ethereum}\label{bitcoin-ed-ethereum}
...
\subsubsection{Le Sfide: Trilemma della
...
\textbf{Privacy:} Le transazioni su ledger pubblici sono visibili a
tutti. Le identità degli utenti possono a volte essere inferite, con
rischio di esposizione di dati sensibili. Bilanciare privacy e
auditabilità è particolarmente delicato nei protocolli \textbf{DeFi}: da
un lato richiedono confidenzialità (transazioni, depositi, prestiti
senza rivelare importi o indirizzi), dall'altro richiedono auditabilità
(verifica del double-spending, conferma della collateralizzazione). Le
...
\textbf{Scalabilità:} Le blockchain tradizionali hanno throughput
limitato e costi energetici elevati. Le soluzioni principali spostano
l'esecuzione \textbf{off-chain}, usando la catena principale solo come
ancora di fiducia (\emph{trust anchor}) per la validazione finale:
...
\section{Reti Overlay e File Sharing}\label{reti-overlay-e-file-sharing}
...
\subsection{Dal Centralizzato al P2P}\label{dal-centralizzato-al-p2p}

\textbf{Napster (2001)} (prima generazione) usava un indice
centralizzato dei file su server dedicati, mentre il trasferimento
avveniva direttamente tra peer. Ha dimostrato che si può servire una
quantità di dati paragonabile a Google con molti meno server, spostando
storage e trasferimento direttamente sugli utenti. All'epoca Google
impiegava circa 15.000 server, Napster circa 100. Il sistema raggiunge
26,4 milioni di utenti nel 2001, con 10 TB di dati (2 milioni di
canzoni, in media 220 per utente). I server si occupano solo di
localizzare chi possiede la risorsa --- la parte meno costosa del
servizio. In questo modello ogni utente è un \textbf{servent} (server +
client): partecipa ``pagando'' con le proprie risorse fisiche, contenuti
o conoscenza.
...
\item
  ...
  dell'amministrazione, nessun collo di bottiglia sulle risorse (storage
  e trasferimento distribuiti tra gli utenti).
\item
  \textbf{Punti di debolezza}: il server resta un singolo punto di
  fallimento e di controllo, necessario per gestire l'intero sistema ---
  esattamente questo lo ha reso vulnerabile agli attacchi legali per
  violazione del copyright, poiché l'analisi dell'indice centralizzato
  permetteva di risalire ai contenuti scambiati tra gli utenti.
\end{itemize}

La seconda generazione (\textbf{Gnutella, Kazaa, BitTorrent}) ha
eliminato ogni punto centrale, distribuendo sia la ricerca che il
trasferimento. \textbf{Gnutella} rimuove anche l'ultimo punto centrale:
nessun indice, connessioni dirette tra peer usate per la ricerca (non
per il download). Il risultato è un sistema senza infrastruttura né
...
\subsection{Reti Overlay}\label{reti-overlay}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Rappresentazione dei due livelli di rete. L'Overlay logico (in alto) è costruito creando connessioni virtuali tra i peer, che fisicamente si traducono in percorsi complessi attraverso l'Underlay IP (in basso).}]{images/Pasted-image-20260407110328.png}}
\caption{Rappresentazione dei due livelli di rete. L'Overlay logico (in
alto) è costruito creando connessioni virtuali tra i peer, che
fisicamente si traducono in percorsi complessi attraverso l'Underlay IP
(in basso).}
\end{figure}
...
\subsubsection{Classificazione degli
...
\subsection{Overlay Non Strutturati}\label{overlay-non-strutturati}
...
\subsubsection{Bootstrapping e
Discovery}\label{bootstrapping-e-discovery}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Processo di Bootstrap. Il peer in ingresso (azzurro) contatta un repository per ottenere la lista dei descrittori, dopodiché stabilisce le connessioni (link virtuali) con gli altri peer disponibili.}]{images/Pasted-image-20260407110433.png}}
\caption{Processo di Bootstrap. Il peer in ingresso (azzurro) contatta
un repository per ottenere la lista dei descrittori, dopodiché
stabilisce le connessioni (link virtuali) con gli altri peer
disponibili.}
\end{figure}
...
\subsubsection{Tecniche di Ricerca
...
\subsection{Overlay Ibridi (SuperPeer)}\label{overlay-ibridi-superpeer}
...
\subsection{Auto-Organizzazione dei Sistemi
P2P}\label{auto-organizzazione-dei-sistemi-p2p}
...
partecipazione che permette all'applicazione di autosostenersi.
...
\section{Overlay Strutturati: DHT e Recupero
Distribuito}\label{overlay-strutturati-dht-e-recupero-distribuito}

In un sistema P2P puro, il recupero dei contenuti può seguire due
paradigmi opposti. Il \textbf{searching} guida la ricerca attraverso gli
attributi del contenuto, in modo analogo a un motore di ricerca (usato
nelle reti non strutturate con flooding). L'\textbf{addressing} assegna
un identificatore univoco a ogni contenuto --- tipicamente il suo hash
crittografico --- e usa quella chiave per recuperarlo. È il fondamento
delle \textbf{Distributed Hash Tables (DHT)}: l'hash del contenuto
diventa la chiave di accesso, e la DHT instrada la query verso il nodo
responsabile di quella chiave con garanzie teoriche precise. Il
compromesso è la rinuncia alle query complesse e il costo del
mantenimento della struttura di indirizzamento. Questo approccio non è
più \emph{location-based} (URL che puntano a una posizione), ma
\emph{content-based} (l'identità del dato è il suo hash, come fa IPFS).

Le DHT si collocano nel punto ottimale di compromesso tra l'approccio
centralizzato e l'approccio non strutturato:
...
Memoria Richiesta
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
Risultato (Falsi negativi)
\end{minipage} \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
\textbf{Flooding} (es. Gnutella) & O(N\^{}2) & O(1) & Falsi negativi
possibili (limite TTL) \\
\textbf{Server Centralizzato} (es. Napster) & O(1) & O(N) & Nessun falso
negativo (ma Single Point of Failure) \\
\textbf{DHT} (es. Chord/Kademlia) & O(log N) & O(log N) & Nessun falso
negativo (risoluzione garantita) \\
\end{longtable}
}
...
Crittografiche}\label{funzioni-hash-crittografiche}

Una \textbf{funzione hash crittografica} mappa dati di lunghezza
arbitraria in un valore di lunghezza fissa (\emph{digest}), garantendo
l'effetto valanga (piccola variazione = digest completamente diverso) e
il determinismo.
...
Ethereum usa \textbf{Keccak}. Queste funzioni sono il mattone base sia
per il consistent hashing delle DHT sia per i puzzle crittografici della
Proof of Work di Bitcoin.

\subsection{Il Problema del Rehashing e il Consistent
Hashing}\label{il-problema-del-rehashing-e-il-consistent-hashing}

Se si distribuiscono i dati su \(N\) nodi con la funzione classica
\(\text{SHA}(x) \bmod N\), quando un nodo viene aggiunto o rimosso,
quasi tutte le chiavi devono essere riassegnate, generando un traffico
di rete insostenibile (fino al 99\% delle chiavi modificate in un
sistema con 10 nodi).
...
gestisce un \textbf{intervallo contiguo} di chiavi dell'anello.
Aggiungere o rimuovere nodi richiede di spostare solo una minoranza di
elementi --- in media \(K/n\) chiavi, dove \(K\) è il totale delle
chiavi e \(n\) il numero di nodi.

\end{calloutdefinition}

\subsection{Costruzione di una DHT: l'Anello (Es.
Chord)}\label{costruzione-di-una-dht-lanello-es.-chord}

La costruzione di una DHT basata su consistent hashing segue tre passi
concettuali: definire uno spazio di identificatori condiviso (modulo
\(2^M\)), connettere i nodi definendo la topologia, e assegnare i dati
basandosi sull'hash.

Consideriamo uno spazio di identificatori \(\{0, \ldots, 15\}\)
organizzato come un anello modulo 16, con cinque nodi in posizioni
hashate:
\[H(a) = 6, \quad H(b) = 5, \quad H(c) = 0, \quad H(d) = 11, \quad H(e) = 2\]

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Topologia ad anello logico di Chord. I nodi sono posizionati sull'anello in base al loro hash. Le frecce indicano i puntatori di routing (finger) che permettono salti di lunghezza esponenzialmente crescente nello spazio degli identificatori.}]{images/mermaid-lezione-3-retrieving-content-e-dht-02.png}}
\caption{Topologia ad anello logico di Chord. I nodi sono posizionati
sull'anello in base al loro hash. Le frecce indicano i puntatori di
routing (finger) che permettono salti di lunghezza esponenzialmente
crescente nello spazio degli identificatori.}
\end{figure}

Il \textbf{successore} \texttt{succ(x)} è il primo nodo sull'anello con
identificatore \(\geq x\), procedendo in senso orario. I dati vengono
memorizzati sul nodo \texttt{succ(k)}, dove \(k = H(\text{key})\).

\begin{callouttip}{Calcolo del successore e Memorizzazione}
...
\item
  Se il dato ``scudo'' ha \(H(\text{scudo}) = 12\), verrà memorizzato
  nel nodo 0.
\item
  Se il nodo 11 abbandona la rete, le chiavi che gestiva passano al suo
  successore (il nodo 0), minimizzando il traffico di riassegnazione.
\end{itemize}

\end{callouttip}

\subsubsection{Node Leave e Node
Failure}\label{node-leave-e-node-failure}

Una \textbf{disconnessione volontaria} prevede il trasferimento
organizzato delle chiavi. Un \textbf{guasto improvviso} (\emph{node
failure}) porta a perdita di dati se non si applicano soluzioni di
resilienza, quali: \textbf{replicazione dei dati} su \(r\) successori,
\textbf{refresh periodico} (gossip), e \textbf{probing periodico} per
trovare percorsi alternativi nelle routing table.

\subsubsection{\texorpdfstring{Finger Table e Routing
\(O(\log N)\)}{Finger Table e Routing O(\textbackslash log N)}}\label{finger-table-e-routing-olog-n}

Se il routing procedesse sequenzialmente sul successore diretto, avremmo
\(O(N)\) hop. Per evitare ciò, protocolli come \textbf{Chord}
introducono la \textbf{Finger Table}:

\begin{calloutdefinition}{Finger Table}

\[\text{finger}[i] = \text{succ}(n + 2^{i-1}) \quad \text{per } i = 1, \ldots, M\]

Ogni nodo mantiene puntatori a distanze esponenzialmente crescenti
(\(+1, +2, +4, +8...\)).

\end{calloutdefinition}

L'algoritmo di lookup inoltra la query al finger con l'identificatore
massimo che non supera la chiave cercata, dimezzando l'intervallo
residuo ad ogni step.

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Algoritmo di routing in Chord. La richiesta viene inoltrata iterativamente al nodo conosciuto più distante che non supera la chiave k, avvicinandosi progressivamente (in modo logaritmico) alla destinazione.}]{images/mermaid-lezione-3-retrieving-content-e-dht-03.png}}
\caption{Algoritmo di routing in Chord. La richiesta viene inoltrata
iterativamente al nodo conosciuto più distante che non supera la chiave
k, avvicinandosi progressivamente (in modo logaritmico) alla
destinazione.}
\end{figure}

\begin{callouttip}{Scala del miglioramento}

Con \(N = 10^6\) nodi, il routing sequenziale richiede fino a 500.000
hop. Con la finger table, i salti scendono a \(\log_2(10^6) \approx\)
\textbf{20 hop}.

\end{callouttip}

\subsection{Load Balancing e Virtual
Server}\label{load-balancing-e-virtual-server}

Il load imbalance si verifica a causa di spazio di indirizzamento non
uniforme, documenti molto pesanti, o hotspot (dati virali e altamente
richiesti). La soluzione standard prevede l'uso di \textbf{virtual
server}: ogni macchina fisica gestisce più ID virtuali sull'anello,
frammentando le responsabilità e distribuendo meglio il carico. Resta
comunque la sfida aperta di gestire hotspot eccezionali.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\section{Kademlia (Protocollo DHT
Avanzato)}\label{kademlia-protocollo-dht-avanzato}

Kademlia è un protocollo DHT (2002) ed è considerato lo standard de
facto su Internet (usato in Ethereum, IPFS, BitTorrent Mainline DHT,
eMule KAD). Le peculiarità che lo rendono superiore ai predecessori
(come Chord) includono: - Apprendimento \textbf{automatico} del routing
osservando il traffico passante. - Supporto al \textbf{parallel routing}
(richieste asincrone parallele per abbassare la latenza). - L'uso della
metrica \textbf{XOR} per la misurazione delle distanze e la disposizione
dei contatti.

\subsection{Spazio degli Identificatori e Metrica
XOR}\label{spazio-degli-identificatori-e-metrica-xor}

Invece di usare una distanza numerica circolare (come Chord), Kademlia
tratta gli identificatori (lunghi \(M\) bit) in un \textbf{trie binario
completo}, dove ogni foglia rappresenta un nodo o una chiave. Per
assegnare le chiavi ai nodi si può immaginare di calcolare il
\emph{Lowest Common Ancestor} nel trie; tuttavia, operativamente,
Kademlia definisce la distanza tramite l'operatore \textbf{XOR}.

\begin{calloutdefinition}{Distanza XOR}

\(d(x, y) = x \oplus y\). Un contenuto viene memorizzato sul nodo con
distanza XOR minima rispetto alla chiave del contenuto stesso. Questa
metrica gode di simmetria (\(d(x,y)=d(y,x)\)) e disuguaglianza
triangolare.

\end{calloutdefinition}

La simmetria è fondamentale: se il nodo A riceve una query da B, B e A
sono esattamente alla stessa distanza. Questo significa che A può
inserire B nella propria routing table gratuitamente (apprendimento
simmetrico), cosa non valida nell'anello asimmetrico di Chord.
L'unidirezionalità della XOR garantisce che le ricerche per la stessa
chiave convergano sempre sullo stesso percorso.

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Albero binario dello spazio delle chiavi di Kademlia. Le foglie rappresentano i nodi. Maggiore è la lunghezza del prefisso condiviso tra due nodi (es. 110 e 111), minore è la loro distanza secondo la metrica XOR.}]{images/Pasted-image-20260407111031.png}}
\caption{Albero binario dello spazio delle chiavi di Kademlia. Le foglie
rappresentano i nodi. Maggiore è la lunghezza del prefisso condiviso tra
due nodi (es. \protect\texttt{110} e \protect\texttt{111}), minore è la
loro distanza secondo la metrica XOR.}
\end{figure}

\subsection{Routing Table e K-Buckets}\label{routing-table-e-k-buckets}

Ogni nodo mantiene una routing table divisa in \(M\) \textbf{K-Buckets}.

\begin{calloutdefinition}{K-Bucket}

Una lista di al massimo \(k\) contatti per il vicinato logico (dove la
distanza XOR \(d \in [2^{i-1}, 2^i)\)). I contatti sono ordinati per
\emph{recency} (dal meno recente al più recente).

\end{calloutdefinition}

Quando arriva un messaggio, il mittente viene aggiunto in coda se il
bucket non è pieno. Se il bucket è pieno, Kademlia esegue un ping al
contatto in testa (il più vecchio); se questo risponde, viene mantenuto
e il nuovo nodo scartato, altrimenti viene sostituito. Questa politica
\textbf{favorisce i nodi vecchi}, sfruttando l'assunto statistico che i
peer connessi da più tempo sono i più affidabili, garantendo anche una
forte resistenza agli attacchi DoS (uno spam di nuovi nodi fasulli non
spiazzerà i vecchi e stabili).

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Partizionamento dello spazio degli ID visto dalla prospettiva del nodo rosso. Ogni sotto-albero corrisponde a un ``k-bucket'' nella routing table del nodo, permettendo di localizzare qualsiasi chiave in O(\textbackslash log N) salti.}]{images/Pasted-image-20260407111152.png}}
\caption{Partizionamento dello spazio degli ID visto dalla prospettiva
del nodo rosso. Ogni sotto-albero corrisponde a un ``k-bucket'' nella
routing table del nodo, permettendo di localizzare qualsiasi chiave in
\(O(\log N)\) salti.}
\end{figure}

\subsection{Lookup Iterativo e
Parallelo}\label{lookup-iterativo-e-parallelo}

Kademlia supporta il lookup iterativo e, soprattutto, \textbf{parallelo}
controllato dal parametro \(\alpha\). Anziché attendere la risposta di
un singolo nodo, il richiedente interroga simultaneamente \(\alpha\)
contatti (es. \(\alpha=3\)).

\begin{itemize}
\tightlist
\item
  Ogni iterazione riduce lo spazio di ricerca (prefix match routing) e
  restituisce fino a \(k\) nuovi nodi vicini.
\item
  Procedendo, il lookup si ferma quando i contatti interrogati non
  restituiscono nodi più vicini di quelli già noti.
\item
  Il routing parallelo abbatte le latenze dovute a timeout e garantisce
  alta affidabilità al churn di rete.
\end{itemize}

\subsection{Gestione dei Dati (Store e
Join)}\label{gestione-dei-dati-store-e-join}

L'operazione \texttt{STORE} prevede la memorizzazione sui \(k\) nodi più
vicini. I dati sono \emph{soft-state}: devono essere
\textbf{ripubblicati periodicamente} (es. ogni 24 ore) dall'originatore.
Kademlia adotta nativamente il \textbf{caching lungo il percorso}: un
dato appena recuperato viene messo in cache temporanea sui nodi
intermedi della ricerca, aiutando a mitigare il problema degli hotspot
per contenuti virali.

Un nuovo nodo entra (Join) contattando un bootstrap node noto ed
eseguendo \texttt{FIND\_NODE} prima del suo stesso ID, poi di ID casuali
distribuiti nello spazio XOR, popolando gradualmente tutti i suoi
K-Buckets in modo fluido e adattivo, superando la rigidità di Chord.

\subsubsection{Sintesi: Chord vs
Kademlia}\label{sintesi-chord-vs-kademlia}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{Metrica} & Distanza numerica (anello) & XOR (Trie Binario) \\
\textbf{Simmetria} & No --- finger table asimmetriche & Sì ---
apprendimento mutuale gratuito \\
\textbf{Routing} & Ricorsivo o iterativo & Iterativo + parallelo
(\(\alpha\) nodi) \\
...
\chapter{Capitolo 2 - Fondamenti Crittografici e Strutture
Dati}\label{capitolo-2---fondamenti-crittografici-e-strutture-dati}

\section{Crittografia per P2P e
...
\subsubsection{Proprietà Fondamentali}\label{proprietuxe0-fondamentali}
...
\subsubsection{Il Paradosso del Compleanno e la Sicurezza
...
\subsubsection{Cryptanalysis}\label{cryptanalysis}
...
\subsubsection{Proprietà Avanzate}\label{proprietuxe0-avanzate}
...
\subsection{Applicazioni delle Funzioni di
...
\subsubsection{Applicazioni blockchain}\label{applicazioni-blockchain}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Concetto di Blockchain. Ogni blocco contiene un riferimento crittografico (hash) al blocco precedente, creando una catena immutabile in cui alterare un blocco invaliderebbe tutti i successivi.}]{images/blockchain_structure.png}}
\caption{Concetto di Blockchain. Ogni blocco contiene un riferimento
crittografico (hash) al blocco precedente, creando una catena immutabile
in cui alterare un blocco invaliderebbe tutti i successivi.}
\end{figure}
...
\subsection{Commitment Scheme}\label{commitment-scheme}

\subsubsection{Motivazione: Sasso-Carta-Forbici
...
\subsubsection{API e Implementazione}\label{api-e-implementazione}
...
\subsection{Search Puzzle (Proof of
...
\subsection{Crittografia Asimmetrica e Firme
...
\subsubsection{Firme Digitali}\label{firme-digitali}
...
\subsubsection{API Standard}\label{api-standard}
...
Authorities}\label{certification-authorities}
...
\subsection{Hash vs Cifratura}\label{hash-vs-cifratura}
...
\subsection{RIPEMD-160 e Bitcoin}\label{ripemd-160-e-bitcoin}
...
\section{Strutture Dati per DHT e
Blockchain}\label{strutture-dati-per-dht-e-blockchain}

\subsection{Hash Pointer}\label{hash-pointer}
...
\subsection{Filtri di Bloom}\label{filtri-di-bloom}

\subsubsection{Il Problema}\label{il-problema}
...
\subsubsection{Costruzione}\label{costruzione}
...
\subsubsection{Probabilità di Falso
...
\subsubsection{Operazioni sugli Insiemi}\label{operazioni-sugli-insiemi}
...
\subsubsection{Applicazioni Reali}\label{applicazioni-reali}
...
\subsubsection{Il Problema dell'Authenticated File
...
\subsubsection{Costruzione}\label{costruzione-1}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Albero di Merkle. I dati originali si trovano nelle foglie (x\_1 \textbackslash ldots x\_8). Ogni nodo intermedio è calcolato come l'hash della concatenazione dei suoi nodi figli, fino alla Root (y\_\{15\}).}]{images/Pasted-image-20260407112004.png}}
\caption{Albero di Merkle. I dati originali si trovano nelle foglie
(\(x_1 \ldots x_8\)). Ogni nodo intermedio è calcolato come l'hash della
concatenazione dei suoi nodi figli, fino alla Root (\(y_{15}\)).}
\end{figure}

\subsubsection{Merkle Proof (Proof of
Inclusion)}\label{merkle-proof-proof-of-inclusion}
...
\subsubsection{Proof of Non-Membership}\label{proof-of-non-membership}
...
\subsection{Trie, Patricia Trie e Merkle Patricia
Trie}\label{trie-patricia-trie-e-merkle-patricia-trie}

\subsubsection{Trie (Prefix Tree / Radix
...
\subsubsection{Patricia Trie}\label{patricia-trie}
...
\subsubsection{Nibble}\label{nibble}
...
\chapter{Capitolo 3 - Introduzione alla Blockchain e
Bitcoin}\label{capitolo-3---introduzione-alla-blockchain-e-bitcoin}
...
\section{Proof of Ownership}\label{proof-of-ownership}
...
Bitcoin non ha il concetto di ``conto'' con un saldo. Esistono solo
trasferimenti e il sistema si basa sugli \textbf{UTXO} (\emph{Unspent
Transaction Output}): le transazioni ricevute e non ancora spese.
Spendere un UTXO lo distrugge e ne crea di nuovi per il destinatario (e
per il resto).

Il saldo mostrato da un wallet è la somma di tutti gli UTXO associati
agli indirizzi dell'utente, calcolata scorrendo la blockchain e
aggregando tutti gli output non spesi.
...
I bitcoin di un utente possono essere distribuiti come UTXO tra
centinaia di transazioni e centinaia di blocchi --- non esiste una
...
\chapter{Capitolo 4 - Bitcoin: Sicurezza, Mining e
Fork}\label{capitolo-4---bitcoin-sicurezza-mining-e-fork}
...
\subsection{Anatomia di un Blocco}\label{anatomia-di-un-blocco}
...
organizzarsi in \textbf{mining pool} --- discusse più avanti in questo
capitolo.
...
\section{Sicurezza: Attacchi e
Vulnerabilità}\label{sicurezza-attacchi-e-vulnerabilituxe0}
...
\subsection{Double Spending e Attacco del
...
validano contemporaneamente generando un fork temporaneo, la Longest
Chain Rule ne renderà valida solo una --- motivo per cui si attendono
solitamente 6 conferme.
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Schema dell'attacco stealth di Double Spending (51\% Attack).}]{images/Pasted-image-20260319164820.png}}
\caption{Schema dell'attacco stealth di Double Spending (51\% Attack).}
\end{figure}
...
\subsection{Attacco Denial of Service}\label{attacco-denial-of-service}
...
\section{L'Evoluzione del Mining}\label{levoluzione-del-mining}
...
\subsection{Solo Mining vs Mining
Pool}\label{solo-mining-vs-mining-pool}
...
\subsubsection{Metodi di Pagamento nelle
Pool}\label{metodi-di-pagamento-nelle-pool}
...
\textbf{Pay Per Share (PPS) e FPPS}
...
\textbf{Pay Per Last N Shares (PPLNS)}
...
\section{Hard e Soft Fork in Bitcoin}\label{hard-e-soft-fork-in-bitcoin}
...
di un attacco (come discusso precedentemente per i fork temporanei). Si
creano così più blocchi validi alla stessa altezza. La rete risolve
l'ambiguità applicando la \textbf{longest chain rule} (la regola della
catena con più lavoro cumulativo): uno dei rami diventa orfano e i suoi
blocchi vengono scartati. Non cambia nessuna regola, non nasce nessun
nuovo asset. È un evento normale nell'operatività quotidiana di Bitcoin.

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Hard fork vs soft fork a confronto: il primo divide la rete in due catene separate, il secondo mantiene la rete unita con regole più restrittive.}]{images/lezione-18-hard-and-soft-forks-in-bitcoin-img-01.jpg}}
\caption{Hard fork vs soft fork a confronto: il primo divide la rete in
due catene separate, il secondo mantiene la rete unita con regole più
restrittive.}
\end{figure}
...
\subsection{Soft Fork}\label{soft-fork}
...
\subsubsection{Accettazione del Fork}\label{accettazione-del-fork}
...
\subsubsection{Come Muore la Vecchia
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Come la vecchia versione muore: i blocchi dei nodi non aggiornati vengono rifiutati dalla catena dominante. I miner seguono ``the chain that pays''.}]{images/lezione-18-hard-and-soft-forks-in-bitcoin-img-02.jpg}}
\caption{Come la vecchia versione muore: i blocchi dei nodi non
...
\subsubsection{I Principali Soft Fork di
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Timeline dei soft fork principali di Bitcoin: da P2SH (2012) per il multisig, a SegWit (2017) per la scalabilità, fino a Taproot (2021) per la privacy.}]{images/lezione-18-hard-and-soft-forks-in-bitcoin-img-03.jpg}}
\caption{Timeline dei soft fork principali di Bitcoin: da P2SH (2012)
per il multisig, a SegWit (2017) per la scalabilità, fino a Taproot
(2021) per la privacy.}
\end{figure}

\subsection{SegWit --- Agosto 2017}\label{segwit-agosto-2017}
...
\subsubsection{Il Problema: Transaction
...
transazioni}): permetteva di alterare il \textbf{TXID} (identificativo)
di una transazione senza modificarne gli effetti reali --- mittente,
destinatario e importo restavano invariati, ma l'ID cambiava. Come se il
numero di tracking di un pacco venisse sostituito in transito.
...
Il txid cambiato rendeva inoltre impossibile per altri protocolli ---
come la Lightning Network --- fare riferimento in modo affidabile a una
transazione non ancora confermata.

\subsubsection{La Soluzione: Separare la Firma dai
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Confronto strutturale: nel blocco Pre-SegWit le firme sono embedded nella transazione; nel blocco SegWit la Witness Data è segregata e non influisce sul calcolo del txid.}]{images/lezione-18-hard-and-soft-forks-in-bitcoin-img-04.jpg}}
\caption{Confronto strutturale: nel blocco Pre-SegWit le firme sono
embedded nella transazione; nel blocco SegWit la Witness Data è
...
\subsubsection{Block Weight e Aumento della
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={In un blocco SegWit la Witness Area (tratteggiata) contiene le firme a peso ridotto. La base data rimane ≤ 1 MB per i vecchi nodi, ma il blocco reale può superarla.}]{images/lezione-18-hard-and-soft-forks-in-bitcoin-img-05.jpg}}
\caption{In un blocco SegWit la Witness Area (tratteggiata) contiene le
firme a peso ridotto. La base data rimane ≤ 1 MB per i vecchi nodi, ma
il blocco reale può superarla.}
\end{figure}

\subsubsection{SegWit: Ricapitolazione}\label{segwit-ricapitolazione}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Ricapitolazione SegWit: il blocco legacy ha firme inline; il blocco SegWit le separa nel Witness. I vecchi nodi vedono solo la base data e rimangono sincronizzati.}]{images/lezione-18-hard-and-soft-forks-in-bitcoin-img-06.jpg}}
\caption{Ricapitolazione SegWit: il blocco legacy ha firme inline; il
blocco SegWit le separa nel Witness. I vecchi nodi vedono solo la base
data e rimangono sincronizzati.}
\end{figure}

\subsection{Taproot --- Novembre 2021}\label{taproot-novembre-2021}
...
\subsubsection{Schnorr Signatures}\label{schnorr-signatures}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Key aggregation con Schnorr: P1, P2, P3 combinano chiavi e firme in un'unica P\_\{agg\} e S\_\{agg\}. La verifica è identica a quella di un firmatario singolo.}]{images/lezione-18-hard-and-soft-forks-in-bitcoin-img-07.jpg}}
\caption{Key aggregation con Schnorr: P1, P2, P3 combinano chiavi e
firme in un'unica \(P_{agg}\) e \(S_{agg}\). La verifica è identica a
...
\subsubsection{MAST --- Merkelized Abstract Syntax
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Da AST a MAST: le condizioni OR (2-of-3 Multisig, Timelock) diventano foglie separate; le condizioni AND (Timelock AND Hash Preimage) rimangono nella stessa foglia.}]{images/lezione-18-hard-and-soft-forks-in-bitcoin-img-08.jpg}}
\caption{Da AST a MAST: le condizioni OR (2-of-3 Multisig, Timelock)
diventano foglie separate; le condizioni AND (Timelock AND Hash
Preimage) rimangono nella stessa foglia.}
\end{figure}

\textbf{La Parte Merkle Tree}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Il Merkle Tree del MAST: ogni condizione di spesa è una foglia. La MAST ROOT impegna tutte le condizioni. Al momento della spesa si rivela solo la foglia usata + il Merkle path.}]{images/lezione-18-hard-and-soft-forks-in-bitcoin-img-09.jpg}}
\caption{Il Merkle Tree del MAST: ogni condizione di spesa è una foglia.
La MAST ROOT impegna tutte le condizioni. Al momento della spesa si
rivela solo la foglia usata + il Merkle path.}
\end{figure}
...
\subsubsection{La Tweaked Public Key}\label{la-tweaked-public-key}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={La tweaked key: combina la chiave pubblica originale con lo script segreto (MAST root) in un'unica chiave che appare normale on-chain ma impegna crittograficamente le condizioni di spesa.}]{images/lezione-18-hard-and-soft-forks-in-bitcoin-img-10.jpg}}
\caption{La tweaked key: combina la chiave pubblica originale con lo
script segreto (MAST root) in un'unica chiave che appare normale
...
\subsubsection{Key Path vs Script Path
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Key path spending: tutti i partecipanti cooperano, producono una firma Schnorr aggregata. Il nodo verifica \textbackslash text\{sig\} rispetto a P\textquotesingle. Indistinguibile da una transazione standard.}]{images/lezione-18-hard-and-soft-forks-in-bitcoin-img-11.jpg}}
\caption{Key path spending: tutti i partecipanti cooperano, producono
una firma Schnorr aggregata. Il nodo verifica \(\text{sig}\) rispetto a
\(P'\). Indistinguibile da una transazione standard.}
\end{figure}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Script path spending: si rivela lo script specifico (tapleaf) e il Merkle path verso la root. Si producono le firme/dati richiesti dallo script. Le altre condizioni rimangono private.}]{images/lezione-18-hard-and-soft-forks-in-bitcoin-img-12.jpg}}
\caption{Script path spending: si rivela lo script specifico (tapleaf) e
il Merkle path verso la root. Si producono le firme/dati richiesti dallo
script. Le altre condizioni rimangono private.}
\end{figure}
...
\subsection{Hard Fork}\label{hard-fork}
...
\subsubsection{Bitcoin Cash --- Agosto
...
\subsubsection{Hard Fork come Strategia di
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={I fork di Bitcoin nel 2017: Bitcoin Cash e Bitcoin Gold sono hard fork che generano nuove catene (BCH, BTG); SegWit è il soft fork che mantiene la catena principale; SegWit2x è un tentativo di hard fork abortito (B2X).}]{images/lezione-18-hard-and-soft-forks-in-bitcoin-img-13.jpg}}
\caption{I fork di Bitcoin nel 2017: Bitcoin Cash e Bitcoin Gold sono
hard fork che generano nuove catene (BCH, BTG); SegWit è il soft fork
che mantiene la catena principale; SegWit2x è un tentativo di hard fork
abortito (B2X).}
\end{figure}

\subsubsection{Hard Fork per Vulnerabilità
...
\chapter{Capitolo 5 - Bitcoin: Scripting e Layer
2}\label{capitolo-5---bitcoin-scripting-e-layer-2}
...
Oltre a questi, esamineremo il trilemma della blockchain, la gestione
dei dati e i client leggeri (SPV).
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Confronto tra firme multiple separate e aggregate.}]{images/Pasted-image-20260407113044.png}}
\caption{Confronto tra firme multiple separate e aggregate.}
\end{figure}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Schema di funzionamento del Pay-To-Script-Hash (P2SH).}]{images/Pasted-image-20260407113120.png}}
\caption{Schema di funzionamento del Pay-To-Script-Hash (P2SH).}
\end{figure}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Schema della rete P2P di Bitcoin non strutturata.}]{images/Pasted-image-20260407113308.png}}
\caption{Schema della rete P2P di Bitcoin non strutturata.}
\end{figure}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Scambio di messaggi VERSION e processo di Handshake tra nodi Bitcoin.}]{images/Pasted-image-20260407113353.png}}
\caption{Scambio di messaggi VERSION e processo di Handshake tra nodi
Bitcoin.}
\end{figure}

\subsection{Gossip Protocol e
Propagazione}\label{gossip-protocol-e-propagazione}
...
Come visto in precedenza, le soluzioni basate interamente on-chain
pongono un limite scalabile, spingendo verso l'utilizzo dei livelli
secondari come la Lightning Network. Vitalik Buterin ha formalizzato
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Esempio di Routing Multi-Hop nella Lightning Network.}]{images/Pasted-image-20260407113700.png}}
\caption{Esempio di Routing Multi-Hop nella Lightning Network.}
\end{figure}
...
\chapter{Capitolo 6 - Ethereum: Macchina Virtuale e
Consenso}\label{capitolo-6---ethereum-macchina-virtuale-e-consenso}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={In Bitcoin lo stato è l'insieme degli UTXO (Unspent Transaction Output). Una transazione consuma degli UTXO esistenti e ne crea di nuovi, generando la transizione S → S'.}]{images/lezione-19-ethereum-accounts-transactions-gas-img-02.jpg}}
\caption{In Bitcoin lo stato è l'insieme degli UTXO (Unspent Transaction
Output). Una transazione consuma degli UTXO esistenti e ne crea di
nuovi, generando la transizione S → S'.}
\end{figure}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Confronto delle capacità: solo gli EOA possono inviare transazioni firmate; solo i Contract Account hanno codice e storage. I contratti possono inviare messaggi (non transazioni firmate) ad altri contratti e crearne di nuovi.}]{images/lezione-19-ethereum-accounts-transactions-gas-img-04.jpg}}
\caption{Confronto delle capacità: solo gli EOA possono inviare
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Formato della transazione EOA→EOA. I campi principali: signature (firma ECDSA del mittente), to (indirizzo del destinatario a 20 byte), amount (valore in wei).}]{images/lezione-19-ethereum-accounts-transactions-gas-img-05.jpg}}
\caption{Formato della transazione EOA→EOA. I campi principali:
\protect\texttt{signature} (firma ECDSA del mittente),
\protect\texttt{to} (indirizzo del destinatario a 20 byte),
\protect\texttt{amount} (valore in wei).}
\end{figure}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Transizione di stato EOA→EOA. Alice (50 ETH, nonce 5) invia 10 ETH a Bob (30 ETH). Lo stato S' vede Alice a 40 ETH (nonce 6) e Bob a 40 ETH. Se il saldo fosse insufficiente (es. Alice=50, Bob=30, send 70) la funzione APPLY restituisce ERROR.}]{images/lezione-19-ethereum-accounts-transactions-gas-img-06.jpg}}
\caption{Transizione di stato EOA→EOA. Alice (50 ETH, nonce 5) invia 10
ETH a Bob (30 ETH). Lo stato S' vede Alice a 40 ETH (nonce 6) e Bob a 40
ETH. Se il saldo fosse insufficiente (es. Alice=50, Bob=30, send 70) la
funzione APPLY restituisce ERROR.}
\end{figure}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Transizione di stato con contract account. Il mittente invia una transazione con value 10 e data ``CHARLIE'' al contratto. Il contratto aggiorna il proprio storage (la lista diventa {[}ALICE, BOB, CHARLIE{]}) e i saldi vengono aggiornati di conseguenza.}]{images/lezione-19-ethereum-accounts-transactions-gas-img-07.jpg}}
\caption{Transizione di stato con contract account. Il mittente invia
una transazione con value 10 e data ``CHARLIE'' al contratto. Il
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Il lifecycle di uno smart contract è una progressione lineare: Creazione → Interazione → Distruzione (opzionale).}]{images/lezione-19-ethereum-accounts-transactions-gas-img-08.jpg}}
\caption{Il lifecycle di uno smart contract è una progressione lineare:
Creazione → Interazione → Distruzione (opzionale).}
\end{figure}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Transazione di creazione: il campo to è vuoto (l'indirizzo del contratto viene generato al deploy), il campo data contiene il bytecode EVM del contratto.}]{images/lezione-19-ethereum-accounts-transactions-gas-img-09.jpg}}
\caption{Transazione di creazione: il campo \protect\texttt{to} è vuoto
(l'indirizzo del contratto viene generato al deploy), il campo
\protect\texttt{data} contiene il bytecode EVM del contratto.}
\end{figure}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Transazione di interazione: to contiene l'indirizzo del contratto, data codifica il metodo da invocare tramite function selector (primi 4 byte del Keccak-256 del prototipo della funzione) + argomenti ABI-encoded.}]{images/lezione-19-ethereum-accounts-transactions-gas-img-10.jpg}}
\caption{Transazione di interazione: \protect\texttt{to} contiene
l'indirizzo del contratto, \protect\texttt{data} codifica il metodo da
invocare tramite function selector (primi 4 byte del Keccak-256 del
prototipo della funzione) + argomenti ABI-encoded.}
\end{figure}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Struttura completa di una transazione Ethereum: nonce, gasLimit, gasPrice, to, value, i tre componenti della firma ECDSA (v, r, s), e data.}]{images/lezione-19-ethereum-accounts-transactions-gas-img-11.jpg}}
\caption{Struttura completa di una transazione Ethereum: nonce,
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Le denominazioni dell'Ether, ciascuna intitolata a un pioniere dell'informatica: wei (unità base), Babbage (10³), Lovelace (10⁶), Shannon (10⁹), Szabo (10¹²), Finney (10¹⁵), Ether (10¹⁸), Grand (10²¹), Megaether (10²⁴).}]{images/lezione-19-ethereum-accounts-transactions-gas-img-12.jpg}}
\caption{Le denominazioni dell'Ether, ciascuna intitolata a un pioniere
dell'informatica: wei (unità base), Babbage (10³), Lovelace (10⁶),
Shannon (10⁹), Szabo (10¹²), Finney (10¹⁵), Ether (10¹⁸), Grand (10²¹),
Megaether (10²⁴).}
\end{figure}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Riepilogo del meccanismo gas. Il mittente specifica gasLimit e gasPrice (es. 100.000 gas × 20 Gwei = 0.002 ETH upfront). L'EVM esegue consumando gas. Se successo: gas non usato rimborsato. Se out-of-gas: tutte le modifiche revertite, gas consumato NON rimborsato (deterrente contro spam). In basso: tabella dei costi gas per le istruzioni principali.}]{images/lezione-19-ethereum-accounts-transactions-gas-img-13.jpg}}
\caption{Riepilogo del meccanismo gas. Il mittente specifica gasLimit e
gasPrice (es. 100.000 gas × 20 Gwei = 0.002 ETH upfront). L'EVM esegue
consumando gas. Se successo: gas non usato rimborsato. Se out-of-gas:
tutte le modifiche revertite, gas consumato NON rimborsato (deterrente
contro spam). In basso: tabella dei costi gas per le istruzioni
principali.}
\end{figure}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Costi gas per operazioni di base: ADD/SUB (3 gas), MUL/DIV (5 gas), ADDMOD/MULMOD (8 gas), operazioni bitwise/confronto (3 gas), operazioni stack POP (2), PUSH/DUP/SWAP (3), MLOAD/MSTORE (3).}]{images/lezione-19-ethereum-accounts-transactions-gas-img-14.jpg}}
\caption{Costi gas per operazioni di base: ADD/SUB (3 gas), MUL/DIV (5
gas), ADDMOD/MULMOD (8 gas), operazioni bitwise/confronto (3 gas),
operazioni stack POP (2), PUSH/DUP/SWAP (3), MLOAD/MSTORE (3).}
\end{figure}

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Costi gas per operazioni avanzate: JUMP (8), JUMPI (10), SLOAD (200), SSTORE (20.000), BALANCE (400), CREATE (32.000), CALL (25.000). Lo storage è deliberatamente costoso per scoraggiare l'uso eccessivo della chain come database.}]{images/lezione-19-ethereum-accounts-transactions-gas-img-15.jpg}}
\caption{Costi gas per operazioni avanzate: JUMP (8), JUMPI (10), SLOAD
(200), SSTORE (20.000), BALANCE (400), CREATE (32.000), CALL (25.000).
Lo storage è deliberatamente costoso per scoraggiare l'uso eccessivo
della chain come database.}
\end{figure}
...
\section{L'offerta di ETH e il Mercato delle
Fee}\label{lofferta-di-eth-e-il-mercato-delle-fee}

\subsection{L'offerta di ETH: dalle origini alla supply
...
\subsection{Il sistema di fee pre-Merge: il First Price
...
\subsection{EIP-1559: la riforma del mercato delle
fee}\label{eip-1559-la-riforma-del-mercato-delle-fee}

L'EIP-1559, attivato con il London Hard Fork (agosto 2021), ha
completamente ridisegnato il meccanismo di fee introducendo quattro
...
\subsubsection{baseFeePerGas: il prezzo di mercato
...
\subsubsection{maxPriorityFeePerGas e
...
\subsubsection{Tre casi numerici}\label{tre-casi-numerici}
...
\tightlist
\item
  Fee totale: 21.000 × (100 + 10) = 2.310.000 gwei = \textbf{0,00231
  ETH}
\item
...
\subsubsection{EIP-1559 e la pressione
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={I quattro livelli dell'architettura Ethereum.}]{images/mermaid-lezione-26-ethereum-2-0-fees-and-tries-01.png}}
\caption{I quattro livelli dell'architettura Ethereum.}
\end{figure}

Il \textbf{Data Layer} comprende le strutture dati che rappresentano lo
stato del sistema --- account, transazioni, ricevute, storage dei
contratti --- attraverso strutture ad albero basate su Merkle Patricia
Trie.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\section{I Trie e il Data Layer}\label{i-trie-e-il-data-layer}

\subsection{Il Merkle Patricia Trie
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Struttura del Beacon Block Header: gestisce slot, proposer, state root e il collegamento all'Execution Layer.}]{images/lezione-26-ethereum-2-0-fees-and-tries-img-01.jpg}}
\caption{Struttura del Beacon Block Header: gestisce slot, proposer,
state root e il collegamento all'Execution Layer.}
\end{figure}

\subsubsection{Execution Layer}\label{execution-layer}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Struttura dell'Execution Layer Block Header: stateRoot, transactionsRoot, receiptsRoot e logsBloom sono le radici dei trie principali.}]{images/lezione-26-ethereum-2-0-fees-and-tries-img-02.jpg}}
\caption{Struttura dell'Execution Layer Block Header: stateRoot,
...
\subsection{I Trie dell'Execution
Layer}\label{i-trie-dellexecution-layer}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Il blocco punta tramite radici hash al World State Trie, al Receipts Trie e al Transactions Trie. Il World State Trie punta all'Account Storage Trie per ogni contratto.}]{images/lezione-26-ethereum-2-0-fees-and-tries-img-03.jpg}}
\caption{Il blocco punta tramite radici hash al World State Trie, al
Receipts Trie e al Transactions Trie. Il World State Trie punta
all'Account Storage Trie per ogni contratto.}
\end{figure}

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Relazione tra Block Header e i quattro trie dell'Execution Layer.}]{images/mermaid-lezione-26-ethereum-2-0-fees-and-tries-02.png}}
\caption{Relazione tra Block Header e i quattro trie dell'Execution
Layer.}
\end{figure}
...
\subsection{Il Transaction Trie}\label{il-transaction-trie}
...
\subsection{Logging e Transaction
Receipt}\label{logging-e-transaction-receipt}

\subsubsection{Perché i log}\label{perchuxe9-i-log}
...
\subsubsection{Struttura del Transaction
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Un contratto Solidity con evento Transfer emette il log quando viene chiamata send(); il log viene registrato nella ricevuta della transazione.}]{images/lezione-26-ethereum-2-0-fees-and-tries-img-04.jpg}}
\caption{Un contratto Solidity con evento Transfer emette il log quando
viene chiamata send(); il log viene registrato nella ricevuta della
transazione.}
\end{figure}
...
\subsubsection{cumulativeGasUsed}\label{cumulativegasused}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Per la transazione N, gasUsed nella ricevuta e la somma del gas di tutte le transazioni da 1 a N. Nell'esempio, la ricevuta della tx 3 riporta 106.000 gwei = 21.000 + 50.000 + 35.000.}]{images/lezione-26-ethereum-2-0-fees-and-tries-img-05.jpg}}
\caption{Per la transazione N, gasUsed nella ricevuta e la somma del gas
di tutte le transazioni da 1 a N. Nell'esempio, la ricevuta della tx 3
riporta 106.000 gwei = 21.000 + 50.000 + 35.000.}
\end{figure}

\subsection{I parametri Indexed e la struttura dei
Log}\label{i-parametri-indexed-e-la-struttura-dei-log}

\subsubsection{Parametri indexed in
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Nel contratto SimpleToken, from e to sono marcati indexed --- vengono salvati nei topics della ricevuta per abilitare ricerche efficienti.}]{images/lezione-26-ethereum-2-0-fees-and-tries-img-06.jpg}}
\caption{Nel contratto SimpleToken, from e to sono marcati indexed ---
...
\subsubsection{La struttura completa di un evento
loggato}\label{la-struttura-completa-di-un-evento-loggato}

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={A sinistra il contratto Token con l'evento Transfer; a destra la ricevuta con topics (event signature + from + to) e data (amount non-indexed).}]{images/lezione-26-ethereum-2-0-fees-and-tries-img-07.jpg}}
\caption{A sinistra il contratto Token con l'evento Transfer; a destra
la ricevuta con topics (event signature + from + to) e data (amount
non-indexed).}
\end{figure}
...
\subsection{LogsBloom: filtraggio efficiente con Bloom
...
\subsubsection{LogsBloom nella ricevuta (livello
transazione)}\label{logsbloom-nella-ricevuta-livello-transazione}

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Per ogni transazione, logsBloom e un Bloom filter costruito sugli indirizzi e i topics dei log con keccak256 e 3 bit impostati nel vettore da 2048 bit.}]{images/lezione-26-ethereum-2-0-fees-and-tries-img-08.jpg}}
\caption{Per ogni transazione, logsBloom e un Bloom filter costruito
sugli indirizzi e i topics dei log con keccak256 e 3 bit impostati nel
vettore da 2048 bit.}
\end{figure}
...
\subsubsection{LogsBloom nel block header (livello
blocco)}\label{logsbloom-nel-block-header-livello-blocco}

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Il logsBloom del block header e l'OR bitwise dei logsBloom di tutte le ricevute del blocco.}]{images/lezione-26-ethereum-2-0-fees-and-tries-img-09.jpg}}
\caption{Il logsBloom del block header e l'OR bitwise dei logsBloom di
tutte le ricevute del blocco.}
\end{figure}
...
\subsection{Il Transaction Receipt Trie e il Storage
Trie}\label{il-transaction-receipt-trie-e-il-storage-trie}

\subsubsection{Il Transaction Receipt
Trie}\label{il-transaction-receipt-trie}

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Il Receipt Trie punta alle ricevute con Gas Used, Logs, Bloom Filter e Status Code. Il sequence diagram mostra l'interazione utente-World State Trie-Receipt Trie durante deploy multipli.}]{images/lezione-26-ethereum-2-0-fees-and-tries-img-10.jpg}}
\caption{Il Receipt Trie punta alle ricevute con Gas Used, Logs, Bloom
Filter e Status Code. Il sequence diagram mostra l'interazione
...
\subsubsection{Il Storage Trie}\label{il-storage-trie}

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Il campo storageRoot punta all'Account Storage Trie con uno slot per ogni contratto. Il sequence diagram mostra tre chiamate successive con lookup e aggiornamento del valore.}]{images/lezione-26-ethereum-2-0-fees-and-tries-img-11.jpg}}
\caption{Il campo storageRoot punta all'Account Storage Trie con uno
slot per ogni contratto. Il sequence diagram mostra tre chiamate
...
\section{Il Consenso: Proof of Stake}\label{il-consenso-proof-of-stake}
...
\subsubsection{Il Problema dei Generali
...
\subsection{Safety e Liveness}\label{safety-e-liveness}
...
identico per principio a quello di Bitcoin. Tuttavia, il mining Bitcoin
ha costi enormi: consuma quantità sproporzionate di energia, e i mining
pool controllano porzioni crescenti della catena, erodendo la
decentralizzazione. Le alternative al PoW includono:
...
Nel settembre 2022, con l'evento noto come \textbf{The Merge}, la rete è
passata al \textbf{Proof of Stake}.

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={The Merge (settembre 2022): la Ethereum Mainnet (PoW) converge con la Beacon Chain (PoS), attiva in parallelo dal 2020. Da quel momento, il consenso è gestito interamente da PoS.}]{images/lezione-19-ethereum-accounts-transactions-gas-img-01.jpg}}
\caption{The Merge (settembre 2022): la Ethereum Mainnet (PoW) converge
con la Beacon Chain (PoS), attiva in parallelo dal 2020. Da quel
momento, il consenso è gestito interamente da PoS.}
\end{figure}

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Timeline degli upgrade del Consensus Layer di Ethereum 2.0, da Beacon Chain a Deneb.}]{images/mermaid-lezione-25-ethereum-consensus-proof-of-stake-01.png}}
\caption{Timeline degli upgrade del Consensus Layer di Ethereum 2.0, da
Beacon Chain a Deneb.}
\end{figure}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Struttura temporale di epoch e slot in Ethereum PoS.}]{images/mermaid-lezione-25-ethereum-consensus-proof-of-stake-02.png}}
\caption{Struttura temporale di epoch e slot in Ethereum PoS.}
\end{figure}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Il protocollo RANDAO: dalla generazione del segreto all'assegnazione dei ruoli.}]{images/mermaid-lezione-25-ethereum-consensus-proof-of-stake-03.png}}
\caption{Il protocollo RANDAO: dalla generazione del segreto
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={LMD GHOST: il ramo superiore (60→50→20) vince sul ramo inferiore più lungo (40→30→30→30) perché ha più stake accumulato.}]{images/mermaid-lezione-25-ethereum-consensus-proof-of-stake-04.png}}
\caption{LMD GHOST: il ramo superiore (60→50→20) vince sul ramo
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Il processo di justification e finalization in Casper FFG: due supermajority consecutive portano C1 alla finalità.}]{images/mermaid-lezione-25-ethereum-consensus-proof-of-stake-05.png}}
\caption{Il processo di justification e finalization in Casper FFG: due
supermajority consecutive portano C1 alla finalità.}
\end{figure}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Confronto sistematico tra Ethereum (2015, Vitalik Buterin) e Bitcoin (2009, Satoshi Nakamoto): use case, modello blockchain, consenso, supply, velocità, sicurezza e community.}]{images/lezione-19-ethereum-accounts-transactions-gas-img-16.jpg}}
\caption{Confronto sistematico tra Ethereum (2015, Vitalik Buterin) e
Bitcoin (2009, Satoshi Nakamoto): use case, modello blockchain,
consenso, supply, velocità, sicurezza e community.}
\end{figure}
...
Ethereum usa {[}{[}Kademlia{]}{]} come protocollo P2P per la peer
discovery, a differenza di Bitcoin che ha sviluppato il proprio
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Confronto su 14 dimensioni: anno di lancio, scopo, consenso, block time, supply model, transaction model, smart contracts, fees, use cases, state model e altro.}]{images/lezione-26-ethereum-2-0-fees-and-tries-img-12.jpg}}
\caption{Confronto su 14 dimensioni: anno di lancio, scopo, consenso,
block time, supply model, transaction model, smart contracts, fees, use
cases, state model e altro.}
\end{figure}
...
\chapter{Capitolo 7 - Token, Storage Decentralizzato e Use
Cases}\label{capitolo-7---token-storage-decentralizzato-e-use-cases}

\section{Fungible e Non Fungible Tokens: Standard
...
cosiddette \textbf{killer application} delle blockchain. Questo capitolo
analizza la distinzione fondamentale tra token fungibili e non
fungibili, i meccanismi contrattuali che li implementano su Ethereum
(standard ERC), lo storage decentralizzato tramite IPFS, e infine
esplora alcune delle principali applicazioni reali degli smart contract.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{Coins e Token: una distinzione
...
\subsection{Token Fungibili}\label{token-fungibili}

\begin{calloutdefinition}{Fungibilit�}
...
\subsubsection{Applicazioni dei token
...
\subsection{Token Non Fungibili (NFT)}\label{token-non-fungibili-nft}

\begin{calloutdefinition}{Non Fungibilit�}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Tassonomia degli asset secondo fungibilità e tangibilità: i token intangibili fungibili includono criptovalute e carbon credit, quelli non fungibili includono arte digitale e copyright.}]{images/mermaid-lezione-22-fungible-e-non-fungible-tokens-erc-standards-01.png}}
\caption{Tassonomia degli asset secondo fungibilità e tangibilità: i
token intangibili fungibili includono criptovalute e carbon credit,
...
\subsection{Cryptographic Tokens e Standard
...
\subsubsection{Perché uno standard?}\label{perchuxe9-uno-standard}
...
\subsection{ERC-20: Standard per Token
...
\subsubsection{Struttura interna: il balance
...
\subsubsection{Funzioni obbligatorie}\label{funzioni-obbligatorie}
...
\subsubsection{Campi opzionali}\label{campi-opzionali}
...
\subsubsection{Meccanismo di trasferimento
...
\subsubsection{Meccanismo di delega
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Flusso del meccanismo allowance: il proprietario approva una quota, il delegato la utilizza con transferFrom.}]{images/mermaid-lezione-22-fungible-e-non-fungible-tokens-erc-standards-02.png}}
\caption{Flusso del meccanismo allowance: il proprietario approva una
quota, il delegato la utilizza con transferFrom.}
\end{figure}
...
\subsubsection{L'interfaccia IERC20 e
...
\subsection{Implementare i Token in
Solidity}\label{implementare-i-token-in-solidity}

\subsubsection{Contratti come classi, istanze e
...
\subsubsection{Ereditarietà}\label{ereditarietuxe0}
...
\subsubsection{OpenZeppelin: non reinventare la
...
\subsection{ERC-721: Standard per NFT}\label{erc-721-standard-per-nft}
...
\subsubsection{safeTransferFrom: perché
...
\subsection{Metadata degli NFT}\label{metadata-degli-nft}
...
\subsubsection{La struttura dei
metadata}\label{la-struttura-dei-metadata}
...
\subsubsection{tokenURI: il collegamento tra on-chain e
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Il flusso dei metadata di un NFT: la tokenURI punta al JSON, che contiene il link IPFS all'immagine e l'array di attributi usati da marketplace e sistemi per filtrare e calcolare la rarità.}]{images/lezione-22-fungible-e-non-fungible-tokens-erc-standards-img-01.jpg}}
\caption{Il flusso dei metadata di un NFT: la \protect\texttt{tokenURI}
...
\subsubsection{Perché gli attributi se c'è
...
\subsubsection{On-chain vs Off-chain: dove archiviare i
...
\subsubsection{Royalties}\label{royalties}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Il meccanismo delle royalties: Alice crea e vende NFT \#123 per 1 ETH con royalty del 5\%. Ad ogni rivendita futura (10 ETH, 10 ETH, 20 ETH), Alice riceve automaticamente il 5\% --- rispettivamente 0.05, 0.5, 0.5, 1 ETH --- senza alcuna azione da parte sua.}]{images/lezione-22-fungible-e-non-fungible-tokens-erc-standards-img-02.jpg}}
\caption{Il meccanismo delle royalties: Alice crea e vende NFT \#123 per
1 ETH con royalty del 5\%. Ad ogni rivendita futura (10 ETH, 10 ETH, 20
ETH), Alice riceve automaticamente il 5\% --- rispettivamente 0.05, 0.5,
0.5, 1 ETH --- senza alcuna azione da parte sua.}
\end{figure}

\subsubsection{Unlockable Content}\label{unlockable-content}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Flusso di verifica per l'unlockable content: la piattaforma chiama ownerOf(tokenId) on-chain e confronta il risultato con il wallet connesso dall'utente.}]{images/mermaid-lezione-22-fungible-e-non-fungible-tokens-erc-standards-03.png}}
\caption{Flusso di verifica per l'unlockable content: la piattaforma
chiama \protect\texttt{ownerOf(tokenId)} on-chain e confronta il
risultato con il wallet connesso dall'utente.}
\end{figure}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{ERC-20 vs ERC-721: il confronto
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={ERC-20 mappa indirizzi a quantità (fungibile); ERC-721 mappa ID unici a proprietari (non fungibile).}]{images/mermaid-lezione-22-fungible-e-non-fungible-tokens-erc-standards-04.png}}
\caption{ERC-20 mappa indirizzi a quantità (fungibile); ERC-721 mappa ID
unici a proprietari (non fungibile).}
\end{figure}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{ERC-1155: Multi-Token
...
\subsubsection{Applicazioni di ERC-1155}\label{applicazioni-di-erc-1155}
...
\subsection{La ``giungla'' degli standard
...
\section{IPFS: Interplanetary File
System}\label{ipfs-interplanetary-file-system}

\subsection{Il Web centralizzato e il problema della
...
\subsubsection{Dal Web location-based al Web
...
\subsection{IPFS: cos'è e da dove
...
\subsection{Lo stack di IPFS a colpo
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Lo stack IPFS: il livello più basso trasporta i dati (network, routing, exchange), quello intermedio li definisce (Merkle-DAG, naming), quello superiore li usa (applicazioni).}]{images/mermaid-lezione-24-ipfs-interplanetary-file-system-01.png}}
\caption{Lo stack IPFS: il livello più basso trasporta i dati (network,
routing, exchange), quello intermedio li definisce (Merkle-DAG, naming),
quello superiore li usa (applicazioni).}
\end{figure}
...
\subsection{Content addressing: l'hashing come
...
\subsubsection{Dal digest al CID (Content
...
\subsection{Il progetto Multiformat}\label{il-progetto-multiformat}
...
\subsubsection{Multibase: come leggere una
...
\subsubsection{Multihash: come leggere un
...
\subsubsection{CID: tutto il Multiformat messo
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Struttura a byte di un CIDv1 (dag-pb, sha2-256): versione, multicodec, funzione di hash, lunghezza, digest.}]{images/mermaid-lezione-24-ipfs-interplanetary-file-system-02.png}}
\caption{Struttura a byte di un CIDv1 (dag-pb, sha2-256): versione,
multicodec, funzione di hash, lunghezza, digest.}
\end{figure}
...
\subsection{IPLD e il Merkle DAG}\label{ipld-e-il-merkle-dag}
...
\subsubsection{Come funziona in
concreto}\label{come-funziona-in-concreto}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Costruzione del Merkle DAG di un file: chunking, hashing, generazione dei CID dei figli e del CID radice.}]{images/mermaid-lezione-24-ipfs-interplanetary-file-system-03.png}}
\caption{Costruzione del Merkle DAG di un file: chunking, hashing,
generazione dei CID dei figli e del CID radice.}
\end{figure}

\subsubsection{Deduplicazione
automatica}\label{deduplicazione-automatica}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Merkle DAG in IPFS (IPLD). CID\_root contiene i CID dei figli CID\_A e CID\_B (entrambe directory). Il file CID\_D è referenziato sia da CID\_A sia da CID\_B: essendo identificato dall'hash del suo contenuto, viene memorizzato una sola volta ma puntato da più genitori. A destra, l'esempio del payload serializzato di CID\_B, che elenca i suoi figli come coppie (name, link).}]{images/lezione-24-ipfs-interplanetary-file-system-img-01.jpg}}
\caption{Merkle DAG in IPFS (IPLD). \protect\texttt{CID\_root} contiene
i CID dei figli \protect\texttt{CID\_A} e \protect\texttt{CID\_B}
(entrambe directory). Il file \protect\texttt{CID\_D} è referenziato sia
da \protect\texttt{CID\_A} sia da \protect\texttt{CID\_B}: essendo
identificato dall'hash del suo contenuto, viene memorizzato una sola
volta ma puntato da più genitori. A destra, l'esempio del payload
serializzato di \protect\texttt{CID\_B}, che elenca i suoi figli come
coppie \protect\texttt{(name,\ link)}.}
\end{figure}

\subsubsection{Proprietà del Merkle
DAG}\label{proprietuxe0-del-merkle-dag}
...
\subsubsection{Ogni nodo può essere
...
\subsection{Il livello di rete: libp2p}\label{il-livello-di-rete-libp2p}
...
\subsubsection{PeerId e multiaddress}\label{peerid-e-multiaddress}
...
\subsection{Il livello di routing: la
...
\subsubsection{Miglioramenti rispetto a
...
\subsection{Il livello di scambio:
...
\subsubsection{Perché non basta la
DHT}\label{perchuxe9-non-basta-la-dht}
...
\subsubsection{Bitswap vs BitTorrent}\label{bitswap-vs-bittorrent}
...
\subsubsection{Come funziona un trasferimento
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Scambio Bitswap: il nodo A chiede un blocco, il nodo B conferma di averlo, A richiede i dati e B li invia. A verifica l'integrità ricalcolando il CID.}]{images/mermaid-lezione-24-ipfs-interplanetary-file-system-04.png}}
\caption{Scambio Bitswap: il nodo A chiede un blocco, il nodo B conferma
di averlo, A richiede i dati e B li invia. A verifica l'integrità
ricalcolando il CID.}
\end{figure}
...
\subsection{Disponibilità dei file: il problema della
...
\subsubsection{Pinning services: Pinata}\label{pinning-services-pinata}
...
\subsubsection{Filecoin: incentivare lo storage con una
...
\subsection{NFT e IPFS}\label{nft-e-ipfs}
...
\subsection{Sintesi su IPFS}\label{sintesi-su-ipfs}
...
\section{Applicazioni Reali con Smart
...
di partenza. L'introduzione degli \textbf{smart contract}, come abbiamo
visto per i token fungibili e non fungibili, ha reso possibile il
passaggio da semplici transazioni finanziarie a sistemi digitali
completamente programmabili on-chain: supply chain, finanza
decentralizzata (DeFi), organizzazioni governate da comunità (DAO) e
identità digitale auto-sovrana (SSI). La chiave di questa espansione è
la \textbf{composabilità}: diversi protocolli possono essere combinati
tra loro come moduli software, creando ecosistemi di applicazioni
interoperabili.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{Caso d'uso: Supply Chain}\label{caso-duso-supply-chain}

\subsubsection{Limiti dell'architettura
...
\subsubsection{Blockchain permissioned per la supply
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Architettura di una blockchain permissioned: ogni organizzazione gestisce un peer node e condivide un ledger immutabile; l'accesso è mediato dal Membership Service Provider (MSP).}]{images/mermaid-lezione-27-applicazioni-reali-con-smart-contracts-01.png}}
\caption{Architettura di una blockchain permissioned: ogni
...
\paragraph{Identificazione dei componenti e
...
\paragraph{Smart contract e automazione della
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Flusso di automazione: dal rilevamento della condizione critica (IoT) all'esecuzione dello smart contract, fino alla registrazione immutabile dell'intervento.}]{images/mermaid-lezione-27-applicazioni-reali-con-smart-contracts-02.png}}
\caption{Flusso di automazione: dal rilevamento della condizione critica
(IoT) all'esecuzione dello smart contract, fino alla registrazione
immutabile dell'intervento.}
\end{figure}

\paragraph{Revoca delle
...
\paragraph{Trasferimento di proprietà e
...
\subsection{NFT: Arte Digitale e
...
Come abbiamo esplorato in precedenza, gli \textbf{NFT} estendono questo
concetto al digitale. Quando Beeple ha mintato il suo NFT per l'opera
\emph{Everydays: The First 5000 Days}, ha creato un token sulla
...
\subsubsection{Stable Coin}\label{stable-coin}
...
\paragraph{Fiat-Backed: ciclo di vita
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Ciclo completo di una stable coin fiat-backed: il minting e il withdrawal coinvolgono il custodian; il trasferimento è puramente on-chain.}]{images/mermaid-lezione-27-applicazioni-reali-con-smart-contracts-03.png}}
\caption{Ciclo completo di una stable coin fiat-backed: il minting e il
withdrawal coinvolgono il custodian; il trasferimento è puramente
on-chain.}
\end{figure}

\paragraph{Stabilità tramite
...
\paragraph{Crypto-Collateralized}\label{crypto-collateralized}
...
\paragraph{Algorithmic Stable Coin}\label{algorithmic-stable-coin}
...
\subsubsection{Lending e Borrowing}\label{lending-e-borrowing}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Modello generale del lending DeFi: il vault gestisce collaterale e debito; l'oracle fornisce prezzi; il liquidator interviene quando il Health Factor scende sotto 1.}]{images/mermaid-lezione-27-applicazioni-reali-con-smart-contracts-04.png}}
\caption{Modello generale del lending DeFi: il vault gestisce
collaterale e debito; l'oracle fornisce prezzi; il liquidator interviene
quando il Health Factor scende sotto 1.}
\end{figure}

\paragraph{Il lender}\label{il-lender}
...
\paragraph{Il liquidatore}\label{il-liquidatore}
...
\paragraph{Blockchain Oracle --- use
...
\subsubsection{Flash Loan}\label{flash-loan}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Flash loan per arbitraggio: borrow → buy on DEX A → sell on DEX B → repay, tutto in un'unica transazione atomica.}]{images/mermaid-lezione-27-applicazioni-reali-con-smart-contracts-05.png}}
\caption{Flash loan per arbitraggio: borrow → buy on DEX A → sell on DEX
B → repay, tutto in un'unica transazione atomica.}
\end{figure}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{Exchange: Centralizzati vs
Decentralizzati}\label{exchange-centralizzati-vs-decentralizzati}

\subsubsection{CEX --- Exchange
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Struttura di un DEX: i liquidity provider alimentano la pool con entrambi i token della coppia; il trader swappa pagando una fee che va agli LP.}]{images/mermaid-lezione-27-applicazioni-reali-con-smart-contracts-06.png}}
\caption{Struttura di un DEX: i liquidity provider alimentano la pool
con entrambi i token della coppia; il trader swappa pagando una fee che
va agli LP.}
\end{figure}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={La curva iperbola xy = k: a sinistra lo stato iniziale (x\_0 = 10, y\_0 = 30, k = 300); a destra lo stato dopo uno swap (x\_1 = 15, y\_1 = 20, k rimane 300). Il punto si muove lungo la curva mantenendo il prodotto costante.}]{images/lezione-27-applicazioni-reali-con-smart-contracts-img-01.jpg}}
\caption{La curva iperbola \(xy = k\): a sinistra lo stato iniziale
(\(x_0 = 10, y_0 = 30, k = 300\)); a destra lo stato dopo uno swap
(\(x_1 = 15, y_1 = 20, k\) rimane 300). Il punto si muove lungo la curva
mantenendo il prodotto costante.}
\end{figure}
...
\paragraph{Il prezzo istantaneo}\label{il-prezzo-istantaneo}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Il prezzo istantaneo di Y è il rapporto y/x, ovvero la pendenza della tangente alla curva \textbackslash psi(x,y) = \textbackslash text\{costante\} nel punto corrente.}]{images/lezione-27-applicazioni-reali-con-smart-contracts-img-02.jpg}}
\caption{Il prezzo istantaneo di Y è il rapporto \(y/x\), ovvero la
pendenza della tangente alla curva \(\psi(x,y) = \text{costante}\) nel
punto corrente.}
\end{figure}

\paragraph{\texorpdfstring{Variazioni di
...
\subsection{Tokenizzazione di Asset Reali
...
\subsection{Rischi Nascosti nel DeFi}\label{rischi-nascosti-nel-defi}
...
\subsubsection{Sandwich Attack}\label{sandwich-attack}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Il sandwich attack: T\_\{A\_1\} (front-run) e T\_\{A\_2\} (back-run) circondano T\_V (vittima) nel blocco proposto. I miner includono nell'ordine corretto grazie al differenziale di gas price.}]{images/lezione-27-applicazioni-reali-con-smart-contracts-img-03.jpg}}
\caption{Il sandwich attack: \(T_{A_1}\) (front-run) e \(T_{A_2}\)
...
\subsubsection{MEV --- Maximum Extractable
...
\subsubsection{Prevenzione degli
...
\subsection{Conclusione: Innovazioni Fondamentali della
...
\chapter{Capitolo 8 - Laboratorio Reti P2P e Analisi di
Bitcoin}\label{capitolo-8---laboratorio-reti-p2p-e-analisi-di-bitcoin}

\section{P2P Networks in Bitcoin ed
Ethereum}\label{p2p-networks-in-bitcoin-ed-ethereum}

Questo capitolo apre la sezione applicativa della parte P2P: anziché
discutere in astratto le proprietà delle reti overlay, si guarda come
due blockchain reali --- Bitcoin ed Ethereum --- abbiano implementato,
ciascuna a modo suo, il livello di comunicazione peer-to-peer su cui
tutto il resto (consenso, propagazione delle transazioni,
sincronizzazione degli stati) si appoggia.

L'obiettivo è fare da ponte tra la teoria (classificazione degli
overlay, Kademlia, DHT) e ciò che si troverà effettivamente installando
un client Bitcoin Core o un nodo \texttt{geth}. La chiave di lettura è
il confronto: \textbf{Bitcoin ha scelto un overlay non strutturato}
basato su gossip, \textbf{Ethereum uno strutturato} basato su Kademlia
--- due risposte molto diverse allo stesso problema, scoprire peer con
cui parlare senza conoscere la topologia globale.
...
\subsection{Il problema di base delle reti
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Le due grandi famiglie di overlay P2P utilizzate dalle principali blockchain.}]{images/mermaid-lezione-5-lab-p2p-networks-in-bitcoin-ed-ethereum-01.png}}
\caption{Le due grandi famiglie di overlay P2P utilizzate dalle
...
\subsection{Bitcoin: overlay non strutturato e
gossip}\label{bitcoin-overlay-non-strutturato-e-gossip}

\subsubsection{Struttura generale}\label{struttura-generale}
...
\subsubsection{Node discovery in
Bitcoin}\label{node-discovery-in-bitcoin}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Handshake iniziale di un nuovo nodo Bitcoin: dal seed DNS all'inserimento attivo nella rete.}]{images/mermaid-lezione-5-lab-p2p-networks-in-bitcoin-ed-ethereum-02.png}}
\caption{Handshake iniziale di un nuovo nodo Bitcoin: dal seed DNS
...
\subsubsection{Monitoraggio della rete}\label{monitoraggio-della-rete}
...
\subsubsection{Attacchi specifici al layer P2P di
...
\subsection{Ethereum: overlay strutturato con
...
\subsubsection{Perché una DHT per il
...
\subsubsection{Come Kademlia è adattato in
...
\subsubsection{Stack a livelli (tiered
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Lo stack tiered di Ethereum: RLPx fornisce serializzazione e trasporto cifrato, DEVp2p gestisce le connessioni, e diversi sub-protocolli usano lo stesso canale in multiplex.}]{images/mermaid-lezione-5-lab-p2p-networks-in-bitcoin-ed-ethereum-03.png}}
\caption{Lo stack tiered di Ethereum: RLPx fornisce serializzazione e
trasporto cifrato, DEVp2p gestisce le connessioni, e diversi
sub-protocolli usano lo stesso canale in multiplex.}
\end{figure}
...
\subsubsection{Node discovery in
pratica}\label{node-discovery-in-pratica}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Le due fasi della scoperta e connessione in Ethereum: UDP per trovare peer, TCP per parlarci in sicurezza.}]{images/mermaid-lezione-5-lab-p2p-networks-in-bitcoin-ed-ethereum-04.png}}
\caption{Le due fasi della scoperta e connessione in Ethereum: UDP per
trovare peer, TCP per parlarci in sicurezza.}
\end{figure}

\subsubsection{Monitorare la rete
...
\subsection{\texorpdfstring{Esempio pratico: connettersi con
\texttt{geth} a una rete
...
\subsubsection{Opzioni rilevanti}\label{opzioni-rilevanti}
...
\subsubsection{Procedura passo-passo}\label{procedura-passo-passo}
...
\subsection{Attacchi alla topologia di
...
\section{Bitcoin con bitcoinj}\label{bitcoin-con-bitcoinj}

Ci dedichiamo ora a Bitcoin visto ``dall'interno'' tramite la libreria
\textbf{bitcoinj}. Dopo aver discusso il layer P2P in astratto, in
questa sezione si scrive vero codice Java che apre connessioni verso la
rete Bitcoin, stampa informazioni sui peer, scarica un blocco di esempio
dalla \textbf{testnet}, genera indirizzi di tutti i tipi previsti dal
protocollo (legacy, SegWit, Taproot) e infine ispeziona il famoso
\textbf{genesis block} estraendone il messaggio di Satoshi.

\begin{calloutnote}{Obiettivi concreti}
...
\subsection{Strumenti e link di
...
\subsection{Esempio di connessione alla
rete}\label{esempio-di-connessione-alla-rete}

Il seguente blocco di codice mostra come bastino poche decine di righe
per collegarsi alla \textbf{testnet3}, scoprire i peer tramite DNS, e
scaricare un blocco noto.

L'\textbf{hash del blocco} di riferimento è preso da
...
\subsubsection{Struttura del programma}\label{struttura-del-programma}
...
\subsubsection{Cosa fa, passo per passo}\label{cosa-fa-passo-per-passo}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Pipeline dell'esempio di connessione: tutto ruota attorno a PeerGroup, l'astrazione bitcoinj che gestisce il pool di connessioni P2P.}]{images/mermaid-lezione-10-lab-bitcoin-con-bitcoinj-01.png}}
\caption{Pipeline dell'esempio di connessione: tutto ruota attorno a
\protect\texttt{PeerGroup}, l'astrazione \protect\texttt{bitcoinj} che
gestisce il pool di connessioni P2P.}
\end{figure}
...
meccanismo di bootstrapping che interroga i seed DNS.
...
\subsection{Indirizzi Bitcoin e loro
tipologie}\label{indirizzi-bitcoin-e-loro-tipologie}

Una volta collegati alla rete, si passa a generare chiavi e indirizzi.
Esistono diversi tipi di indirizzo in Bitcoin mainnet:
...
\subsubsection{Generare un indirizzo da una
...
\subsubsection{Esercizio 1 --- Vanity
...
\subsection{Inspezione del genesis
block}\label{inspezione-del-genesis-block}

L'ultima parte di questa fase è l'ispezione del blocco 0 di Bitcoin ---
...
\subsubsection{Codice per leggere il
...
\subsubsection{Il messaggio di Satoshi}\label{il-messaggio-di-satoshi}
...
\subsubsection{Cosa estraiamo dal
...
\begin{calloutnote}{Risultati della prima fase con bitcoinj}

\begin{itemize}
\tightlist
\item
  Connessione alla \textbf{testnet Bitcoin} e scaricamento di un blocco
  richiesto per hash
\item
  \textbf{Generazione chiavi ECDSA} e derivazione indirizzi legacy
  (\texttt{P2PKH}) o SegWit nativi (\texttt{P2WPKH})
\item
  Esercizio su vanity address tramite brute-force
\item
  Ispezione diretta del \textbf{genesis block} di Bitcoin con il
  messaggio di Satoshi decodificato
\end{itemize}

\end{calloutnote}

\section{Bitcoin Transactions e
Scripts}\label{bitcoin-transactions-e-scripts}

Entriamo ora nel cuore della serializzazione del protocollo: si scrive
codice Java per \textbf{ispezionare una transazione campo per campo},
...
\begin{calloutnote}{Obiettivi di analisi delle transazioni}

\begin{itemize}
\tightlist
\item
  Scrivere una funzione \texttt{printTxInfo} che stampa in modo
  leggibile tutti i campi di una \texttt{Transaction} di bitcoinj
\item
...
\item
  ...
  hex}, deserializzarle, ispezionarle e verificare il risultato
\item
...
\subsection{Riferimenti utili}\label{riferimenti-utili}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={I campi di una transazione Bitcoin. marker/flag e witness data sono presenti solo nelle transazioni SegWit.}]{images/mermaid-lezione-11-lab-bitcoin-transactions-e-scripts-01.png}}
\caption{I campi di una transazione Bitcoin.
\protect\texttt{marker}/\protect\texttt{flag} e
\protect\texttt{witness\ data} sono presenti solo nelle transazioni
SegWit.}
\end{figure}
...
\subsection{\texorpdfstring{\texttt{printTxInfo} --- stampare una
transazione in modo
...
Il seguente codice stampa una \texttt{Transaction} di bitcoinj in
formato umano, costringendo a distinguere i casi (coinbase vs regolare,
con vs senza witness) e a ricavare gli indirizzi dai rispettivi script.
...
\subsubsection{Cosa vale la pena notare}\label{cosa-vale-la-pena-notare}
...
\item
  ...
  tenta di decodificare lo \texttt{scriptPubKey} in un indirizzo.
\end{itemize}
...
\subsection{\texorpdfstring{\texttt{printScriptAsOpCodes} ---
decodificare uno script
...
\subsubsection{Logica del parser}\label{logica-del-parser}
...
hash sono 20 byte e le firme stanno sotto i 75. Per script più complessi
bisognerebbe estendere il parser.

\end{calloutwarning}

\subsubsection{\texorpdfstring{Esempio: decodifica di uno
\texttt{scriptPubKey}
...
\subsection{OP\_RETURN: dati arbitrari sulla
...
\subsection{Leggere transazioni reali}\label{leggere-transazioni-reali}

Si possono scaricare transazioni vere dalla blockchain di produzione e
farle digerire al parser:

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  Prendere un TXID noto
\item
...
\section{Script Classification e
blk.dat}\label{script-classification-e-blk.dat}

Facciamo un salto di livello: invece di guardare transazioni \textbf{una
alla volta}, scriviamo un \textbf{parser dell'intera blockchain} che
legge i file \texttt{blk*.dat} in cui Bitcoin Core salva i blocchi sul
disco e produce statistiche aggregate (numero di transazioni, coinbase,
witness, distribuzione dei tipi di script, indirizzi non
decodificabili).

Prima si completa il parser di script costruendo le utility che
\textbf{classificano} un output in una delle categorie standard e che da
uno \texttt{scriptPubKey} \textbf{ricavano l'indirizzo} nel formato
appropriato. Poi si applica il parser all'intera blockchain usando
\texttt{BlockFileLoader} di bitcoinj.
...
\subsection{\texorpdfstring{\texttt{ScriptTypeCustom} --- la tassonomia
dei tipi di
script}{ScriptTypeCustom --- la tassonomia dei tipi di script}}\label{scripttypecustom-la-tassonomia-dei-tipi-di-script}

Per rendere il risultato del parser manipolabile si definisce una classe
...
\subsection{\texorpdfstring{\texttt{ScriptParser.typeFromOut} ---
...
Ogni tipo di script ha una \textbf{forma canonica} in termini di opcode;
basta verificare se lo \texttt{scriptPubKey} ha esattamente quella
forma.
...
\subsection{\texorpdfstring{\texttt{ScriptParser.addrFromOut} ---
ricavare l'indirizzo dallo
...
l'encoding giusto all'hash giusto.
...
\subsection{Bitcoin Core --- i file
...
stesso ha scritto. Per analisi di grandi quantità di blocchi è l'unico
approccio praticabile.

\subsection{\texorpdfstring{\texttt{BCParser} --- analizzare l'intera
blockchain}{BCParser --- analizzare l'intera blockchain}}\label{bcparser-analizzare-lintera-blockchain}

Costruiamo una classe \texttt{BCParser} che scorre tutti i file blk.dat
e raccoglie statistiche.
...
per fare il chaining.

\end{calloutwarning}

\subsubsection{\texorpdfstring{\texttt{parseBlockExact} --- processare
un singolo
...
dove: - \textbf{\texttt{inputs}} (vuoto se coinbase):
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Pipeline di BCParser: dai file blk.dat al CSV-like con statistiche aggregate sulla blockchain intera.}]{images/mermaid-lezione-13-lab-script-classification-e-blk-dat-01.png}}
\caption{Pipeline di \protect\texttt{BCParser}: dai file blk.dat al
...
È fattibile ma richiede molto più lavoro e memoria.

\end{callouttip}

\section{Anonimato e
Deanonimizzazione}\label{anonimato-e-deanonimizzazione}

In questa sezione si affrontano due temi correlati: il completamento
della pipeline di parsing del \texttt{blk.dat} con la gestione degli
UTXO, e l'analisi dell'anonimato in Bitcoin. Il filo conduttore è che la
blockchain è pubblica e persistente: tutto ciò che avviene su di essa è
osservabile, e l'unica protezione offerta nativamente è la
\textbf{pseudonimia}, non l'anonimato vero.

\subsection{Completamento della pipeline: dal blk.dat al CSV con
...
\subsection{Anonimato in Bitcoin: pseudonimia e i suoi
limiti}\label{anonimato-in-bitcoin-pseudonimia-e-i-suoi-limiti}

Bitcoin non è anonimo: è \textbf{pseudonimo}. In un sistema anonimo, le
transazioni non sono riconducibili ad alcun attore. In Bitcoin, ogni
transazione è firmata con la chiave privata dell'indirizzo mittente, e
l'intera storia delle transazioni è pubblica e permanente nella
blockchain. L'identità reale di chi controlla un indirizzo non è
direttamente visibile, ma le sue azioni (movimenti di fondi, tempi,
importi) sono completamente tracciate.
...
reale controlla più pseudonimi. Le euristiche di clustering riescono
spesso a raggruppare questi indirizzi ricondizionandoli allo stesso
utente.
...
si articola in tre stadi progressivi:

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Pipeline di deanonimizzazione: dalla blockchain al grafo delle identità reali.}]{images/mermaid-lezione-17-lab-bitcoin-anonimato-e-deanonimizzazione-01.png}}
\caption{Pipeline di deanonimizzazione: dalla blockchain al grafo delle
identità reali.}
\end{figure}
...
\subsubsection{Clustering euristico}\label{clustering-euristico}

Le euristiche si basano sull'osservazione del comportamento reale degli
utenti Bitcoin e sui vincoli tecnici del protocollo. Generano falsi
positivi e falsi negativi. La prassi è preferire la riduzione dei falsi
positivi a scapito di un aumento dei falsi negativi.
...
output, quale è il destinatario e quale è il resto?
...
\item
  ...
  marketplace raccolgono dati KYC e indirizzi di consegna fisici. Se le
  autorità accedono a questi log, l'associazione indirizzo--identità è
  diretta.
\item
...
\subsubsection{Network listening}\label{network-listening}
...
Le assunzioni di base sono due: il primo nodo a diffondere una nuova
transazione nella rete è con alta probabilità il suo creatore, e il
creatore, avendo firmato la transazione, conosce le chiavi private degli
input e quindi è il proprietario degli indirizzi corrispondenti.
...
\subsubsection{Topology discovery}\label{topology-discovery}
...
attack) e può causare effetti simili a un DoS sulla rete.

\subsection{Dataset: Users Graph 2013 e caso
...
\subsection{Tracciamento dei furti: taint
...
\textbf{blacklisting} sugli indirizzi del ladro.
...
\subsection{Contromisure per la
privacy}\label{contromisure-per-la-privacy}

\subsubsection{CoinJoin e CoinShuffle}\label{coinjoin-e-coinshuffle}
...
\subsubsection{Tecniche di offuscamento}\label{tecniche-di-offuscamento}
...
\subsubsection{Mixer}\label{mixer}
...
\chapter{Capitolo 9 - Laboratorio Sviluppo Smart Contract in
Solidity}\label{capitolo-9---laboratorio-sviluppo-smart-contract-in-solidity}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Confronto tra EOA e Contract: solo l'EOA possiede una chiave privata e paga il gas. Il Contract riceve il proprio indirizzo come hash di sender+nonce alla creazione.}]{images/lezione-20-lab-solidity-img-01.jpg}}
\caption{Confronto tra EOA e Contract: solo l'EOA possiede una chiave
privata e paga il gas. Il Contract riceve il proprio indirizzo come hash
di sender+nonce alla creazione.}
\end{figure}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Il block header punta via hash a tre trie: World State, Receipts e Transactions. L'Account State contiene nonce, balance, storageRoot e codeHash; lo storageRoot punta a sua volta all'Account Storage Trie.}]{images/lezione-20-lab-solidity-img-02.jpg}}
\caption{Il block header punta via hash a tre trie: World State,
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={I blocchi N e N+1 mostrano come i Merkle Patricia Trie si propaghino tra blocchi: le foglie gialle rappresentano gli account modificati, i nodi condivisi rimangono inalterati.}]{images/lezione-20-lab-solidity-img-03.jpg}}
\caption{I blocchi N e N+1 mostrano come i Merkle Patricia Trie si
...
normali ma il gas consumption è anch'esso predefinito.
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Remix IDE: pannello di deploy a sinistra, editor con Counter.sol al centro, log delle transazioni in basso. Si vedono le chiamate a inc, dec, get con i rispettivi hash e gas usato.}]{images/lezione-20-lab-solidity-img-04.jpg}}
\caption{Remix IDE: pannello di deploy a sinistra, editor con
Counter.sol al centro, log delle transazioni in basso. Si vedono le
chiamate a \protect\texttt{inc}, \protect\texttt{dec},
\protect\texttt{get} con i rispettivi hash e gas usato.}
\end{figure}

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Remix IDE (dettaglio): a sinistra il pannello di deploy, al centro l'editor con Counter.sol, in basso i log delle transazioni con hash, gas usato e valori restituiti.}]{images/lezione-21-lab-solidity-avanzato-img-01.jpg}}
\caption{Remix IDE (dettaglio): a sinistra il pannello di deploy, al
centro l'editor con Counter.sol, in basso i log delle transazioni con
hash, gas usato e valori restituiti.}
\end{figure}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Il contratto Counter: variabile di stato uint256 public count, funzioni get() (view), inc() e dec() (public). La funzione dec() fallisce se count = 0 per underflow aritmetico.}]{images/lezione-20-lab-solidity-img-05.jpg}}
\caption{Il contratto Counter: variabile di stato
\protect\texttt{uint256\ public\ count}, funzioni \protect\texttt{get()}
(view), \protect\texttt{inc()} e \protect\texttt{dec()} (public). La
funzione \protect\texttt{dec()} fallisce se \protect\texttt{count\ =\ 0}
per underflow aritmetico.}
\end{figure}

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Counter.sol (dettaglio del codice sorgente nell'editor).}]{images/lezione-21-lab-solidity-avanzato-img-02.jpg}}
\caption{Counter.sol (dettaglio del codice sorgente nell'editor).}
\end{figure}
...
\section{Advanced Solidity: Vulnerabilities e Contract
Upgrading}\label{advanced-solidity-vulnerabilities-e-contract-upgrading}

Affrontiamo ora due temi che separano il codice didattico dal codice
production-ready: le \textbf{vulnerabilità tipiche} degli smart contract
e le \textbf{strategie di aggiornamento} per contratti che, per natura,
...
\begin{callouttip}{Filosofia della sicurezza}
...
\begin{calloutnote}{Riferimenti della sicurezza}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Schema dell'attacco di phishing basato su tx.origin: l'EOA della vittima resta tx.origin lungo tutta la catena di chiamate, quindi il controllo ingannevole passa nonostante il vero chiamante sia un contratto ostile.}]{images/mermaid-lezione-23-lab-advanced-solidity-vulnerabilities-e-upgrading-01.png}}
\caption{Schema dell'attacco di phishing basato su
\protect\texttt{tx.origin}: l'EOA della vittima resta
\protect\texttt{tx.origin} lungo tutta la catena di chiamate, quindi il
controllo ingannevole passa nonostante il vero chiamante sia un
contratto ostile.}
\end{figure}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Il flusso di una reentrancy classica: la chiamata esterna restituisce il controllo al contratto malevolo, che rientra nella funzione prima che lo stato sia aggiornato.}]{images/mermaid-lezione-23-lab-advanced-solidity-vulnerabilities-e-upgrading-02.png}}
\caption{Il flusso di una reentrancy classica: la chiamata esterna
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Tassonomia delle strategie di upgrading: si parte dalla scelta se introdurre o meno un proxy, e all'interno di ciascuna famiglia si distinguono approcci monolitici e modulari.}]{images/mermaid-lezione-23-lab-advanced-solidity-vulnerabilities-e-upgrading-03.png}}
\caption{Tassonomia delle strategie di upgrading: si parte dalla scelta
se introdurre o meno un proxy, e all'interno di ciascuna famiglia si
distinguono approcci monolitici e modulari.}
\end{figure}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Pattern proxy: l'utente interagisce sempre con lo stesso indirizzo (il proxy), ma la logica eseguita è quella del contratto puntato, sostituibile dall'owner.}]{images/mermaid-lezione-23-lab-advanced-solidity-vulnerabilities-e-upgrading-04.png}}
\caption{Pattern proxy: l'utente interagisce sempre con lo stesso
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Pattern diamond: un unico indirizzo esposto all'utente, ma le funzioni sono implementate da più facet. La mappatura funzione → facet è aggiornabile, permettendo di sostituire, aggiungere o rimuovere facet nel tempo.}]{images/mermaid-lezione-23-lab-advanced-solidity-vulnerabilities-e-upgrading-05.png}}
\caption{Pattern diamond: un unico indirizzo esposto all'utente, ma le
funzioni sono implementate da più facet. La mappatura funzione → facet è
aggiornabile, permettendo di sostituire, aggiungere o rimuovere facet
nel tempo.}
\end{figure}
...
\section{Mappa concettuale}\label{mappa-concettuale}

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Struttura complessiva dei pattern di upgrading: da un lato i pattern di attacco e le relative difese, dall'altro l'evoluzione dei pattern architetturali per superare l'immutabilità degli smart contract.}]{images/mermaid-lezione-23-lab-advanced-solidity-vulnerabilities-e-upgrading-06.png}}
\caption{Struttura complessiva dei pattern di upgrading: da un lato i
pattern di attacco e le relative difese, dall'altro l'evoluzione dei
pattern architetturali per superare l'immutabilità degli smart
contract.}
\end{figure}
...
\item
  ...
  gamificato
\item
  \texttt{ethereum.org/developers/docs/smart-contracts/security} ---
  guida ufficiale Ethereum ai disaster recovery plans e sicurezza.
\item
  \texttt{ethernaut.openzeppelin.com} --- CTF di OpenZeppelin per
  esercitarsi sui bug classici.
\end{itemize}
